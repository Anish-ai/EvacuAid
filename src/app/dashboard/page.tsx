...
const TYPE_ICON: Record<string, React.ReactNode> = {
  ...
  Maintenance: <Wrench      className="h-3.5 w-3.5 text-gray-400" />,
...
const SEV_COLORS: Record<Severity, { bg: string; text: string; border: string; pill: string }> = {
  ...
  low:      { bg: 'rgba(148,163,184,0.08)', text: '#94A3B8', border: 'rgba(148,163,184,0.2)', pill: 'bg-slate-500/10 text-slate-400 border border-slate-500/20' },
}
...
const ROLE_COLORS: Record<string, string> = {
  'Global Admin':   '#818CF8', 'Fire Dept': '#EF4444',
  'Medical Staff':  '#F97316', 'Security Team': '#EAB308',
  'Hazmat Unit':    '#A855F7', 'IT Dept': '#3B82F6', 'Operations': '#94A3B8',
}
...
function StatCard({
  ...
  return (
    <Link href={href} style={{ textDecoration: 'none' }}>
      <div style={{
        background: '#111c32', borderRadius: 12,
...
export default function DashboardPage() {
  ...
  const timelineItems = [
    ...incidents.slice(0, 3).map(i => ({
      ...
      time: i.time,
      color: i.severity === 'critical' ? '#EF4444' : i.severity === 'high' ? '#F97316' : '#818CF8',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <p style={{ color: '#475569', fontSize: 13, margin: '4px 0 0' }}>
            ...
            {mapLinkedActive > 0 && (
              <Link href="/map" style={{ color: '#818CF8', fontWeight: 600, marginLeft: 10, textDecoration: 'none' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{
            display: 'flex', alignItems: 'center', gap: 7, padding: '7px 14px',
            background: '#111c32', borderRadius: 8, border: '1px solid rgba(99,102,241,0.2)',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
        ...
        <StatCard label="Unread Alerts" value={unreadCount()} sub={`${ackRate}% ACK rate overall`}
          icon={<Bell className="h-5 w-5" />} accentColor="#818CF8" href="/notifications" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}>
        ...
        <div style={{ background: '#111c32', borderRadius: 12, border: '1px solid rgba(99,102,241,0.15)', padding: '16px 18px' }}>
              ...
              return (
                <Link key={inc.id} href="/incidents" style={{ textDecoration: 'none' }}>
                  <div style={{
                    ...
                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#162040' }}
                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = `${sev.bg}` }}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        {TYPE_ICON[inc.type] ?? <AlertTriangle className="h-3.5 w-3.5 text-gray-400" />}
                        <span style={{ fontFamily: 'monospace', fontSize: 11, color: '#818CF8', fontWeight: 600 }}>{inc.id}</span>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: '#F1F5F9', fontWeight: 500, marginBottom: 2 }}>
                      {inc.type}
                    </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{
            background: '#111c32', borderRadius: 12,
            border: criticalCount() > 0 ? '1px solid rgba(239,68,68,0.3)' : '1px solid rgba(99,102,241,0.15)',
            ...
            <div style={{ padding: '0' }}>
              {criticalIncidents.slice(0, 2).map((inc, i) => (
                <Link key={inc.id} href="/incidents" style={{ textDecoration: 'none' }}>
                  <div style={{
                    ...
                    {inc.description && (
                      <div style={{ fontSize: 11, color: '#94A3B8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{inc.description}</div>
                    )}
                    {/* Countdown only for the first critical */}
                    {i === 0 && (
                      <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(0,0,0,0.3)', borderRadius: 6, padding: '4px 10px', border: '1px solid rgba(239,68,68,0.2)' }}>
                        <span style={{ fontSize: 10, color: '#94A3B8' }}>Auto-Escalation in:</span>
              ...
              <Link href="/incidents" style={{ display: 'block', padding: '10px 16px', textAlign: 'center', fontSize: 11, fontWeight: 600, color: '#818CF8', textDecoration: 'none', borderTop: '1px solid rgba(99,102,241,0.12)', background: 'rgba(99,102,241,0.04)' }}>
          ...
          {/* Pending Tasks */}
          <div style={{ background: '#111c32', borderRadius: 12, border: '1px solid rgba(99,102,241,0.15)', padding: '16px 18px', flex: 1 }}>
              ...
              {pendingTasks.map(task => (
                <Link key={task.id} href="/tasks" style={{ textDecoration: 'none' }}>
                  <div style={{
                    ...
                    background: 'var(--routing-bg-surface, #0d1526)', border: '1px solid rgba(99,102,241,0.12)',
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                        <span style={{ fontFamily: 'monospace', fontSize: 10, color: '#818CF8', fontWeight: 600 }}>{task.id}</span>
                        <span style={{ fontSize: 9, color: '#475569' }}>→</span>
                        <span style={{ fontFamily: 'monospace', fontSize: 10, color: '#818CF8' }}>{task.incidentId}</span>
                      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                return (
                  <Link key={n.id} href="/notifications" style={{ textDecoration: 'none' }}>
                    <div style={{
                      ...
                      background: '#0d1526', border: `1px solid ${n.escalated ? 'rgba(239,68,68,0.3)' : 'rgba(99,102,241,0.15)'}`,
                      display: 'flex', alignItems: 'flex-start', gap: 10,
                    }}
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#162040' }}
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = '#0d1526' }}
                    >
                      <div style={{ width: 6, height: 6, borderRadius: '50%', background: n.ack ? 'transparent' : '#6366F1', boxShadow: n.ack ? 'none' : '0 0 6px rgba(99,102,241,0.6)', flexShrink: 0, marginTop: 4 }} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 2 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span style={{ fontSize: 11, fontWeight: 600, color: '#F1F5F9' }}>{n.user}</span>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 2 }}>
                          {inc && (
                            <span style={{ fontSize: 9, fontWeight: 700, color: '#818CF8', background: 'rgba(99,102,241,0.15)', border: '1px solid rgba(99,102,241,0.25)', padding: '1px 5px', borderRadius: 4 }}>
          ...
          <Link href="/map" style={{ textDecoration: 'none' }}>
            <div style={{
              background: 'linear-gradient(135deg, #111c32 0%, #0d1526 100%)',
              borderRadius: 12, border: '1px solid rgba(99,102,241,0.25)', padding: '16px 18px',
              cursor: 'pointer', transition: 'all 0.2s', position: 'relative', overflow: 'hidden',
            }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(99,102,241,0.5)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 0 30px rgba(99,102,241,0.15)' }}
            ...
            >
              <div style={{ position: 'absolute', top: -20, right: -20, width: 80, height: 80, borderRadius: '50%', background: 'rgba(99,102,241,0.06)', pointerEvents: 'none' }} />
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 20 }}>🗺</span>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#F1F5F9' }}>Building Map</div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {[
                  ...
                  { label: 'Hazard Types', value: 5, color: '#818CF8' },
                  { label: 'Algorithms', value: 2, color: '#34D399' },
                ].map(({ label, value, color }) => (
                  <div key={label} style={{ flex: 1, background: 'rgba(0,0,0,0.3)', borderRadius: 7, padding: '8px 10px', border: '1px solid rgba(99,102,241,0.1)' }}>
                    <div style={{ fontSize: 16, fontWeight: 800, color }}>{value}</div>
                    <div style={{ fontSize: 9, color: '#475569', marginTop: 1 }}>{label}</div>
                  </div>
                ))}
              </div>
              <div style={{ marginTop: 10, fontSize: 11, color: '#818CF8', fontWeight: 500, display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      <div style={{ background: '#111c32', borderRadius: 12, border: '1px solid rgba(99,102,241,0.15)', overflow: 'hidden' }}>
        <div style={{ padding: '10px 16px', borderBottom: '1px solid rgba(99,102,241,0.12)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <Activity className="h-3.5 w-3.5 text-indigo-400" />
          <span style={{ fontSize: 12, fontWeight: 700, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>System Timeline</span>
          <TrendingUp className="h-3 w-3 text-green-400 ml-auto" />
          <span style={{ fontSize: 10, color: '#34D399', fontWeight: 600 }}>Live</span>
        </div>
        <div style={{ height: 38, display: 'flex', alignItems: 'center', overflow: 'hidden', position: 'relative', background: 'rgba(0,0,0,0.3)' }}>
          <div style={{ position: 'absolute', left: 0, width: 40, height: '100%', background: 'linear-gradient(to right, #0d1b2a, transparent)', zIndex: 10 }} />
          <div style={{ position: 'absolute', right: 0, width: 40, height: '100%', background: 'linear-gradient(to left, #0d1b2a, transparent)', zIndex: 10 }} />
          <div style={{ display: 'flex', gap: 48, whiteSpace: 'nowrap', animation: 'marquee 25s linear infinite', fontFamily: 'monospace', fontSize: 12, color: '#94A3B8' }}>
