...
const TYPE_ICON: Record<string, React.ReactNode> = {
  ...
  Maintenance: <Wrench className="h-4 w-4 text-gray-400" />,
...
function IncidentDetailPanel({ incident, onClose }: { incident: Incident; onClose: () => void }) {
  ...
  return (
    <div style={{
      ...
      background: '#111c32', borderLeft: '1px solid rgba(99,102,241,0.2)',
      display: 'flex', flexDirection: 'column', boxShadow: '-8px 0 40px rgba(0,0,0,0.5)',
    }}>
      <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(99,102,241,0.2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div style={{ fontSize: 11, color: '#475569', fontFamily: 'monospace', marginBottom: 2 }}>{incident.id}</div>
          <div style={{ fontSize: 17, fontWeight: 700, color: '#F1F5F9' }}>{incident.type} Incident</div>
        </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {incident.description && (
          <div>
            ...
            <div style={{ fontSize: 13, color: '#94A3B8', lineHeight: 1.6, background: '#0d1526', borderRadius: 8, padding: '10px 12px', border: '1px solid rgba(99,102,241,0.15)' }}>
              {incident.description}
            </div>
          </div>
        )}
            ...
            {(['New', 'In Progress', 'Acknowledged', 'Resolved'] as IncidentStatus[]).map(s => (
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${incident.status === s ? STATUS_COLORS[s] + '60' : 'rgba(99,102,241,0.15)'}`,
  ...
  return (
    <div className="space-y-4 flex flex-col h-full relative">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white mb-1">Incidents Hub</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
        {[
          { label: 'Total Incidents',   value: incidents.length,                                                        color: '#818CF8', icon: '📋' },
          { label: 'Active',            value: activeCount(),                                                           color: '#F59E0B', icon: '🚨' },
          { label: 'Critical',          value: criticalCount(),                                                         color: '#EF4444', icon: '🔥' },
          { label: 'Map-Linked Active', value: incidents.filter(i => i.mapLinked && i.status !== 'Resolved').length,    color: '#818CF8', icon: '🗺' },
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        <input
          ...
          style={{
            background: '#0d1526', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 7,
            padding: '6px 12px', fontSize: 12, color: '#F1F5F9', outline: 'none',
          }}
        />
        ...
        {(['all', 'critical', 'high', 'medium', 'low'] as FilterSev[]).map(s => (
          <button key={s} onClick={() => setFilterSev(s)} style={{
            ...
            background: filterSev === s ? (s === 'all' ? 'rgba(99,102,241,0.2)' : SEV_COLORS[s as Severity]?.bg ?? 'rgba(99,102,241,0.2)') : 'transparent',
            color: filterSev === s ? (s === 'all' ? '#818CF8' : SEV_COLORS[s as Severity]?.text ?? '#818CF8') : '#475569',
            border: `1px solid ${filterSev === s ? (s === 'all' ? 'rgba(99,102,241,0.4)' : SEV_COLORS[s as Severity]?.border ?? 'rgba(99,102,241,0.4)') : 'rgba(99,102,241,0.1)'}`,
          }}>
            {s.toUpperCase()}
          </button>
        ))}
      ...
      {/* Table */}
      <div style={{
        flex: 1, overflowY: 'auto', border: '1px solid rgba(99,102,241,0.15)',
        borderRadius: 10, background: '#111c32',
      }}>
        <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid rgba(99,102,241,0.15)', background: '#0d1526' }}>
          ...
          <tbody>
            {filtered.map(incident => {
              ...
              return (
                ...
                >
                  <td style={{ padding: '12px 16px', fontFamily: 'monospace', color: '#818CF8', fontWeight: 600 }}>{incident.id}</td>
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, color: '#94A3B8' }}>
                      {TYPE_ICON[incident.type] ?? <AlertTriangle className="h-4 w-4 text-gray-500" />}
                      <span style={{ color: '#F1F5F9', fontWeight: 500 }}>{incident.type}</span>
                    </div>
                  </td>
                  <td style={{ padding: '12px 16px' }}>
                    <Badge label={incident.severity.toUpperCase()} color={sev.text} bg={sev.bg} border={sev.border} />
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    {incident.mapLinked ? (
                      <span style={{ fontSize: 11, color: '#818CF8', display: 'flex', alignItems: 'center', gap: 4 }}>🗺 Map</span>
                    ) : (
